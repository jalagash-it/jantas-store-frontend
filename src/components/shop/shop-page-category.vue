<template>
  <div>
    ...
    <template v-if="!isLoading">
      ...
      <CategoryLayout :columns="columns" :sidebar-position="sidebarPosition">
        <template v-slot:sidebar>
          ...
          >
            ...
            <CategorySidebarItem
              v-if="offcanvas !== 'always'"
              class="d-none d-lg-block"
            >
              <WidgetProducts
                title="Новейшие Продукты"
...
export default class ShopPageCategory extends Vue {
  ...
  readonly columns!: ShopPageCategoryColumns;
  @Prop({ type: String, default: () => "grid" })
  readonly viewMode!: ShopPageCategoryViewMode;
  @Prop({ type: String, default: () => "start" })
  readonly sidebarPosition!: ShopPageCategorySidebarPosition;
  ...
  get breadcrumb(): ILink[] {
    ...
    if (this.category) {
      getCategoryParents(this.category).forEach((parent) => {
        breadcrumb.push({
          title: parent.name,
          url: this.$url.category(parent),
        });
      });

      breadcrumb.push({
        title: this.category.name,
        url: this.$url.category(this.category),
      });
    }
